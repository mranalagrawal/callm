...
export const useLayout = defineStore({
  ...
  actions: {
    setMappedTopbar(data: ITopBarMetaObject) {
      if (!data) {
        this.$nuxt.$sentry.captureException(`Missing data on setMappedTopbar: ${this.$nuxt.$cmwStore.settings.store}`)
        return
      }
    ...
    setMappedFooter(data: IFooterMetaObject) {
      if (!data) {
        this.$nuxt.$sentry.captureException(`Missing data on setMappedFooter: ${this.$nuxt.$cmwStore.settings.store}`)
        return
      }
      ...
      this.$patch({
        footer: {
          ...
          mobileApps: data.apps.references.nodes
          ...
          paymentMethods: data.paymentMethods.references.nodes
            ?.filter(({ image }) => image)
            ?.map(({ image }) => ({
              altText: image?.reference?.image?.altText || '',
              url: image?.reference?.image?.url || '',
          ...
          socialLinks: data.socialLinks.references.nodes
            ?.filter(({ link, image }) => link && image)
            ?.map(({ link, image }) => ({
              altText: image?.reference?.image?.altText || '',
              link: link?.value || '',
    ...
    async loadTopbar() {
      await this.$nuxt.$graphql.default.request(getTopBar, {
        ...
        .catch(err => this.$nuxt.$handleApiErrors(`Catch on loadTopbar from GraphQL: ${err}`))
    },

    async loadFooter() {
      await this.$nuxt.$graphql.default.request(getFooter, {
        lang: this.$nuxt.app.i18n.locale.toUpperCase(),
        handle: {
          handle: 'footer',
          type: 'footer',
        ...
        .catch(err => this.$nuxt.$handleApiErrors(`Catch on loadFooter from GraphQL: ${err}`))
    },
  },
})
